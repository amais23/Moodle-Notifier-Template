...
- `MOODLE_USERNAME`：登入 Moodle 的學號（例如 `"41200000S"`）。
- `LINE_USER_ID`：接收推播的 LINE 目標 ID（步驟 0 取得的 `U` 開頭字串）。
...
進入你 GitHub 倉庫的 `Settings` -> 左側欄 `Secrets and variables` -> `Actions`，點擊綠色按鈕 **New repository secret**，新增以下兩個變數：
1. `MOODLE_PASSWORD`：你的 Moodle 登入密碼。
2. `LINE_TOKEN`：步驟 0 取得的 LINE Channel Access Token。
