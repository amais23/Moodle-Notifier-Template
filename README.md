...
* **🤖 LINE Bot 一鍵式快捷助理 (v2 升級版)**
  ...
  - **Flex Message 氣泡卡片**：
    - `/courses` — 查看本學期修習的監控課程。課程卡片附有 **「📢 公告」** 與 **「📁 講義」** 快捷按鈕。
      - **「📁 講義」按鈕**：改為顯示主題 (Section) 選單，點擊特定主題後可展開教材下載清單（支援單一主題最多 15 個檔案，解除原先最多 8 個的限制並提供分主題排版）。
      - 底部配置 **「✨ 顯示最近消息 (7天內)」** 按鈕：點選後並發抓取並集中展示 7 天內新公告、新增講義與即將到期之未繳作業。
    - `/assignments` (或 `/todo`) — 以 Carousel 跑馬燈卡片顯示**未繳交的作業、截止時間與緊急倒數（依緊急程度變色）**，卡片下方附有 **「🔍 查詢繳交狀態」** Postback 互動按鈕。
    - `/grades` — 查詢本學期修習科目的成績明細。**自動過濾排除無成績紀錄科目**，保持介面簡潔。
